...
static int auto_focus_mode(sensor_t *sensor)
{
    if (af_initialized) {
        return 0;
    }
    ...
    uint8_t state = 0x8F;
    int rc = 0;
    rc = set_reg(sensor, 0x3000, 0xff, 0x20);  //reset
    if (rc < 0) return -1;

    for (i = 0; i < sizeof(OV5640_AF_Config); i++) {
        rc = set_reg(sensor, addr, 0xff, OV5640_AF_Config[i]);
        if (rc < 0) return -1;
        addr++;
    }

    set_reg(sensor, OV5640_CMD_MAIN, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_ACK, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_PARA0, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_PARA1, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_PARA2, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_PARA3, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_PARA4, 0xff, 0x00);
    set_reg(sensor, OV5640_CMD_FW_STATUS, 0xff, 0x7f);
    set_reg(sensor, 0x3000, 0xff, 0x00);

    i = 0;
    do {
        state = get_reg(sensor, 0x3029, 0xff);
    ...
    ret = set_reg(sensor, OV5640_CMD_MAIN, 0xff, 0x01);
    if (ret != 0) return ret;

    ret = set_reg(sensor, OV5640_CMD_MAIN, 0xff, 0x08);
    if (ret != 0) return ret;

    do {
        temp = get_reg(sensor, OV5640_CMD_ACK, 0xff);
        retry++;
        if (retry > 1000) return 2;
        vTaskDelay(5 / portTICK_PERIOD_MS);
    } while (temp != 0x00);

    ret = set_reg(sensor, OV5640_CMD_ACK, 0xff, 0x01);
    if (ret != 0) return ret;

    ret = set_reg(sensor, OV5640_CMD_MAIN, 0xff, AF_CONTINUE_AUTO_FOCUS);
    if (ret != 0) return ret;
...
int ov5640_init(sensor_t *sensor)
{
    ...
    sensor->auto_focus_mode = auto_focus_mode;
