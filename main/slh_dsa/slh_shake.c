                        ...
                        size_t ctx_sz)
{
  ...
  slh_shake256_init(&slh_sha3);
  slh_shake_update(&slh_sha3, r, n);
  slh_shake_update(&slh_sha3, var->pk_seed, n);
  slh_shake_update(&slh_sha3, var->pk_root, n);
  ...
  if (ctx_sz != SLH_CTX_SZ_NO_CONTEXT)
  {
    buf[0] = 0;
    buf[1] = ctx_sz & 0xFF;
    slh_shake_update(&slh_sha3, buf, 2);
    slh_shake_update(&slh_sha3, ctx, ctx_sz);
  }
  slh_shake_update(&slh_sha3, m, m_sz);

  slh_shake_out(&slh_sha3, h, var->prm->m);
}
...
static void shake_f(slh_var_t *var, uint8_t *h, const uint8_t *m1)
{
  ...
  slh_shake256_init(&slh_sha3);
  slh_shake_update(&slh_sha3, var->pk_seed, n);
  slh_shake_update(&slh_sha3, (const uint8_t *)var->adrs->u8, 32);
  slh_shake_update(&slh_sha3, m1, n);

  slh_shake_out(&slh_sha3, h, n);
}
...

{
  ...
  slh_shake256_init(&slh_sha3);
  slh_shake_update(&slh_sha3, var->sk_prf, n);
  slh_shake_update(&slh_sha3, opt_rand, n);
                    ...
                    const uint8_t *m2)
{
  ...
  slh_shake_update(&slh_sha3, (const uint8_t *)var->adrs->u8, 32);
  slh_shake_update(&slh_sha3, m1, n);
  slh_shake_update(&slh_sha3, m2, n);
                        ...
                        uint32_t i, uint32_t s)
{
  ...
  for (j = 0; j < s; j++)
  {
    ...
    slh_keccak_f1600(ks); /* permutation */
  }
  memcpy(tmp, ks, n);
}
