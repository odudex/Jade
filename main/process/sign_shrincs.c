...
void sign_shrincs_process(void* process_ptr)
{
    ...
    if (!shrincs_sign_stateful((const uint8_t*)message, msg_len, &sk, &state, sig_output, jade_progress_adapter, &shrincs)) {
        jade_process_reject_message(process, CBOR_RPC_INTERNAL_ERROR, "Sign failed");
        goto cleanup;
    }

    jade_process_reply_to_message_bytes(&process->ctx, sig_output, N + WOTS_SIGN_LEN + state.q * N);
    JADE_LOGI("Success");

cleanup:
    JADE_LOGI("sign_shrincs reached cleanup, returning");
