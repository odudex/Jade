...
/* WOTS+C: number of bytes used to store the grinding counter in a signature. */
#define XMSS_WOTS_CTR_BYTES 4
...
typedef struct {
    ...
    /* WOTS+C: fixed target that the base_w message digits must sum to. */
    unsigned int wots_target_sum;
